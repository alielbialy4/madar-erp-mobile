...
export function InventoryListToolbar({
  ...
  const c = useColors();
  const activeCount = countActiveInventoryFilters(surface, filters);
  const chips = useMemo(
    () => getActiveInventoryFilterChips(surface, filters, warehouseName),
    [surface, filters, warehouseName],
  );
