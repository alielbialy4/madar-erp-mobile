...
import {
  getOfflineQueue,
  removeOfflineMutations,
  requeueOfflineMutations,
...
export function FailedMutationsSheet({ visible, onClose, onChanged }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setRows(queue.filter((item) => item.status === 'failed'));
  }, []);
  ...
  const retryOne = async (row: OfflineMutation) => {
    if (!isOnline || busy) return;
    setBusy(true);
    try {
      await requeueOfflineMutations(new Set([row.id]));
      await syncAll();
      await notify();
      toast.success(t('header.retry'));
    } catch {
      toast.error(t('header.syncFailedOther', { count: 1 }));
    } finally {
      setBusy(false);
    }
  };

  const dismissOne = async (row: OfflineMutation) => {
    const ok = await dialog.confirm({
      title: t('header.dismiss'),
      message: `${row.method} ${row.url}`,
      destructive: true,
    ...
    setBusy(true);
    try {
      await removeOfflineMutations(new Set([row.id]));
      await notify();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <AppBottomSheet visible={visible} onClose={onClose} title={t('header.failedOtherOps')} size="form">
      {busy ? (
      ...
      {rows.length === 0 ? (
        <AppText style={[styles.empty, { color: c.textMuted }]}>{t('header.emptyFailedMutations')}</AppText>
      ) : (
        rows.map((row) => (
